...
class UserController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($query)
                ...
                ->editColumn('date_start', function ($row) {
                    return $row->start_date ? date('Y-m-d H:i', strtotime($row->start_date)) : '';
                })
                ->editColumn('date_end', function ($row) {
                    return $row->end_date ? date('Y-m-d H:i', strtotime($row->end_date)) : '';
                })
    ...
    public function plantingData($id)
    {
        $plantingData = UserSensor::with('sensorKey', 'useSensor')
            ->where('user_id', $id)
            ->whereHas('useSensor', function ($query) {
